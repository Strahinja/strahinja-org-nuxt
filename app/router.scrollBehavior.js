...
if (process.client) {
  ...
  if (
      /************************************************************************
       * BUGFIX BELOW: Default Nuxt's router.scrollBehavior.js doesn't check if
       * the Page object has options field, blindly accessing scrollToTop from
       * it, occasionally giving error. Example case: accessing nonexistant
       * route when having generated pages (SSG), for example
       * http://myserver/asdf -- [SR]
       ************************************************************************/
      // One of the children set `scrollToTop`
      Pages.some(Page => Page && Page.options && Page.options.scrollToTop) ||
      // scrollToTop set in only page without children
      (Pages.length < 2 && Pages.every(Page => Page && Page.options && Page.options.scrollToTop !== false))
  ) {
    position = { x: 0, y: 0 }
  }
