...
function dd(element, settings) {
	...
	var has_handler = function (name) {
		...
		var evs;
		if (typeof $._data == "function" && typeof obj !== 'undefined' && typeof obj[0] !== 'undefined') { //1.8
	...
	var _scrollToIfNeeded = function (opt) {
		...
		if (opt.length > 0) {
			...
			//console.log('_scrollToIfNeeded: pos = ' + pos);
			if (pos > ch) {
				var top = pos + $("#" + childid).scrollTop() - (ch/2);
				//console.log('_scrollToIfNeeded: top = ' + top);
	...
	var _adjustOpen = function () {
		...
		/*
		console.log('_adjustOpen: slimScroll.height = ' + $('#' + childid + ' .slimScrollDiv').height());
		console.log('_adjustOpen: ul.height = ' + $('#' + childid + ' ul').height());
		console.log('_adjustOpen: ul.html = ' + $('#' + childid + ' ul').html());
		*/

		var top = $("#" + id).height(); //this close so its title height
		/*
		console.log('_adjustOpen: '
			+ 'pos = '
			+ pos
			+ ', mH = '
			+ mH
	...
	this.on = function (type, fn, args) {
		if (typeof args !== 'undefined')
		{
			//console.log('CALLING on(): args = ');
			//console.log(args);
			$("#" + _element).on(type, args, fn);
		}
		else
		{
			//console.log('CALLING on(): args = undefined');
			$("#" + _element).on(type, fn);
		}
	};
	this.off = function (type, fn, args) {
		if (typeof args !== 'undefined')
		{
			$("#" + _element).off(type, args, fn);
		}
		else
		{
